...
class Buyer_Seller(Trader):
    ...
    #if the trader is down, the nodes will have to remove the trader from their list so that future request donot go to the that trrader
    def removeTrader(self,traderId):
        logging.basicConfig(filename='example_peer.log',level=logging.DEBUG)
        if ("Peer"+str(traderId)) in self.traderList:
            self.traderList.remove("Peer"+str(traderId))
        #logging.info(str(self.node_id) + ": trader " + str(traderId)+" removed from the trader list. Trader list now is:" + str(self.traderList))
    ...
    #this function is to get stock of the specific trader. This function calls getStock from the trader service
    def getStock(self):
        logging.basicConfig(filename='example_peer.log',level=logging.DEBUG)
        #logging.info(str(self.node_id)+" inside getStock - " + str(self.isTrader))
        if self.isTrader == True:
            stock = super().getStock()
            #logging.info(str(self.node_id)+" has stock as follow: salt---" + str(stock))
            return stock
        return [0,0,0]

    #register stock with the database service. This is a trader function overriden in the child process 
    def registerStock(self,salt,boar,fish):
        logging.basicConfig(filename='example_peer.log',level=logging.DEBUG)
        if self.isTrader == True:
            super().registerStock(salt,boar,fish)
    ...
    def registerPeer(self):
        ...
        try:
            with Pyro4.Daemon(host="127.0.0.1", port=(5000+self.node_id)) as daemon:
                ...
                #logging.info("Before true req loop "+str(self.node_id))
                while True:
                    if self.isTrader == True:
                        #logging.info(str(self.node_id) + ": inside for loop. is trader" + str(self.isTrader) + ": list of trader" + str(self.traderList))
                        super().__init__(self.node_id,self.neighbors,self.total_nodes,self.traderList)
                        super().startTrader()
