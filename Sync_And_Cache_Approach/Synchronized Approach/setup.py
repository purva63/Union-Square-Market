...
for i in range(int(n)):
    ...
    if i==0 or (i!=1 and decider==0):
        role = 'buyer'
        args_lst = ["python", role+str(i)+"/"+"Buyer_Seller.py",str(n),str(int(i)),role]
    else:
        role = 'seller'
        args_lst = ["python", role+str(i)+"/"+"Buyer_Seller.py",str(n),str(int(i)),role]
    
    os.mkdir(role+str(i))
    shutil.copy('Buyer_Seller.py',role+str(i))
    shutil.copy('Trader.py', role+str(i))
    time.sleep(2)
    print(args_lst)
